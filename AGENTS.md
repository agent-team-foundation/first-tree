...
- `e2e/` is the designated directory for versioned Momentic cases and optional tooling for running a QA case's journey in a browser, not a layer of its own: it needs an external Momentic account, resolves steps through a hosted model, and is not a CI gate. The committed case stays the contract. New or materially changed Momentic cases may merge only after human review and revision; see [e2e/README.md](e2e/README.md).
