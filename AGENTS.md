...
- `skills/first-tree-seed/SKILL.md` — one-time bootstrap workflow for a brand-new, still-empty Context Tree.
- `skills/first-tree-write/SKILL.md` — source-backed Context Tree write workflow for selecting the correct target before editing from a PR / doc / note / pasted source.
- The runtime-generated briefing carries the unconditional First Tree surface (chat rules, daemon lifecycle guardrails, GitHub attention, and CLI namespace map). Retired helper payloads may remain in history, but the shipped runtime skill set is the three entries above.
...
- `skills/` — repo-local shipped runtime skill payloads (`first-tree-read`, `first-tree-seed`, `first-tree-write`)
