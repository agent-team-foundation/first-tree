...
- `e2e/` holds browser-level Momentic journeys for flows that span Web + Server + database and have no single package to live in (today: registration and onboarding). It is executable, unlike `packages/qa` prose cases, but it is **not deterministic** — steps resolve from natural-language descriptions via a hosted model and some assertions are model-evaluated — so it is **not a CI gate** and a red run is a signal to investigate, not a merge blocker. It needs an external Momentic account. Do not move a check here that Vitest could assert. Promoting it to a gate is a separate decision; see [e2e/README.md](e2e/README.md).
