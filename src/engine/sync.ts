...
async function prepareProposalBranch(
  shellRun: ShellRun,
  treeRoot: string,
  binding: TreeBindingState,
  drift: DriftReport,
  ...
  dryRun: boolean,
  now: () => Date,
): Promise<PreparedPr | null | false> {
  // Returns PreparedPr on success, null if skipped, false on error
  ...
  const allowEmptyCommit = !hasMissFiles && hasSupplements;
  if (allowEmptyCommit) {
  ...
  if (stagingCheck.code === 0 && !allowEmptyCommit) {
  ...
  const commitResult = await shellRun("git", commitArgs, {
    cwd: treeRoot,
  });
  ...
  const sourcePrRef = group.sourcePrNumber !== null
    ? `${drift.ownerRepo.owner}/${drift.ownerRepo.repo}#${group.sourcePrNumber}`
    : "unlinked commits";
  const sourcePrUrl = group.sourcePrNumber !== null
    ? `https://github.com/${drift.ownerRepo.owner}/${drift.ownerRepo.repo}/pull/${group.sourcePrNumber}`
    : null;
  const bodyLines = [
    // Machine-readable marker (invisible to humans)
    `<!-- gardener:sync · source_pr=${group.sourcePrNumber ?? "unlinked"} · source_repo=${drift.ownerRepo.owner}/${drift.ownerRepo.repo} · tree_sha=${shortSha} -->`,
    "",
    `**🌳 gardener:sync** · source: \`${sourcePrRef}\``,
    "",
    "> [!NOTE]",
    "> **Tree Sync** — this PR proposes context tree updates based on a",
    `> recently merged source PR. Generated by [gardener](https://github.com/agent-team-foundation/repo-gardener).`,
    "",
    "### Source",
    "",
    sourcePrUrl
      ? `**${group.sourcePrTitle}** ([${sourcePrRef}](${sourcePrUrl}))${group.sourcePrAuthor ? ` by @${group.sourcePrAuthor}` : ""}`
      : `Unlinked commits in \`${drift.ownerRepo.owner}/${drift.ownerRepo.repo}\``,
    "",
    ...(misses.length > 0 ? [
      "### New nodes",
      "",
      ...misses.map((p) => {
        const nodePath = p.path.replace(/\/NODE\.md$/i, "");
        return `- \`${nodePath}/NODE.md\` — ${p.rationale}`;
      }),
      "",
    ] : []),
    ...(supplements.length > 0 ? [
      ...
      "",
      "The following existing nodes should be updated to cover new information:",
      "",
      ...supplements.map((p) => [
        `<details>`,
    ...
    "---",
    "",
    "<sub>Commands: <code>@gardener re-sync</code> · <code>@gardener approve</code> · <code>@gardener ignore</code></sub>",
    "",
    `<sub>🌳 Generated by <a href="https://github.com/agent-team-foundation/repo-gardener">gardener sync</a> · powered by <a href="https://github.com/agent-team-foundation/first-tree">First-Tree</a></sub>`,
  ];
  ...
  if (dryRun) {
    ...
    return { branch, prTitle, prBody: bodyLines.join("\n"), group };
  }
...
export async function runSync(
  ...
  for (const drift of driftReports) {
    ...
    // Phase 2: Apply — create branches locally (serial), then push + open PRs (parallel)
    if (flags.apply) {
      const toApply = classifiedPrs.filter((c) => c.filtered.length > 0);
      console.log(
        `\nApplying ${toApply.length} tree PR(s)...`,
      );

      // Step 2a: Create all branches and commits locally (fast, no network)
      ...
      for (const { pr, filtered, written } of toApply) {
        ...
        const result = await prepareProposalBranch(
          shellRun,
          repo.root,
          drift.binding,
          drift,
      ...
      for (const p of prepared) {
        ...
        if (verifyResult.code !== 0) {
          ...
          } else if (verifyResult.stderr.trim() !== "") {
            console.error(`    ${verifyResult.stderr.trim()}`);
          } else if (verifyResult.stdout.trim() !== "") {
            console.error(`    ${verifyResult.stdout.trim()}`);
          } else {
            console.error("    verify exited non-zero without error details.");
          }
          console.log(`  \u23ED Skipping this PR due to verify failure.`);
        } else {
          verified.push(p);
        }
        const restoreVerify = await shellRun("git", ["checkout", originalRef], { cwd: repo.root });
        if (restoreVerify.code !== 0) {
          console.error(`\u274C could not return to ${originalRef}: ${restoreVerify.stderr.trim()}`);
          return 1;
        }
      }
      ...
      if (!flags.dryRun && verified.length > 0) {
        ...
        for (let i = 0; i < verified.length; i += PR_CONCURRENCY) {
          ...
          prCreateFailures += batchResults.filter((success) => !success).length;
        }
        if (prCreateFailures > 0) {
          console.error(`\u26A0 ${prCreateFailures} PR(s) failed to open. Check errors above.`);
          return 1;
        }
      ...
      } else if (flags.dryRun && verified.length > 0) {
