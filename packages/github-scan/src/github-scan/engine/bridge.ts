...
import { type SpawnOptions, type SpawnSyncReturns, spawnSync } from "node:child_process";
...
function walkUpFor(startUrl: string, predicate: (dir: string) => boolean): string | null {
...
export function resolveFirstTreePackageRoot(startUrl: string = import.meta.url): string {
...
export function resolveStatuslineBundlePath(startUrl: string = import.meta.url): string {
  const dev = resolvePackageRootViaRequire(startUrl);
  if (dev !== null) {
...
export type SpawnFn = (command: string, args: readonly string[], options: SpawnOptions) => SpawnSyncReturns<Buffer>;
 ...
 * safe fallback).
 */
export function spawnInherit(command: string, args: readonly string[], deps: SpawnTargetDeps = {}): number {
