...
export function parseDoctorArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (a?.startsWith("--allow-repo=")) out.allowRepo = a.slice("--allow-repo=".length);
  }
  return out;
}
...
export async function runDoctor(argv: readonly string[] = [], options: RunDoctorOptions = {}): Promise<number> {
  ...
  try {
    ...
    scopeLine = identityHasRequiredScope(identity) ? "ok" : "missing repo/notifications";
  } catch (err) {
  ...
  write(`allowed repos: ${filter.isEmpty() ? "all" : filter.displayPatterns()}`);
