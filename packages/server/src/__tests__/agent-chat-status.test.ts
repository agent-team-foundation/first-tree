...
describe("agent-chat-status", () => {
  ...
  describe("withTurnNarration — sticky narration on the working activity", () => {
    ...
    it("keeps only the compact one-line preview on the status projection", () => {
      const out = withTurnNarration(base, "Plan:\n1. read\n2. edit");
      expect(out?.turnText).toBe("Plan: 1. read 2. edit");
    });

    it("caps long status narration at the one-line preview length", () => {
