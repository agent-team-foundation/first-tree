...
function mrPayload(reviewers: { username: string }[]) {
  return {
    ...
    object_attributes: {
      ...
      url: "https://gitlab.internal/Acme/Fenced/-/merge_requests/31",
      state: "opened",
    },
  };
}
...
async function holdIngressAfterDurableCard(
  ...
  await withGitlabIngressFence(app.db, fixture.connection.connectionId, endpoint.connection.tokenHash, async (tx) => {
