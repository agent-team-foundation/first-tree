...
} from "../../services/context-reviewer-publisher.js";
...
export async function agentContextReviewRunRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Params: { chatId: string; runId: string } }>(
    "/:chatId/context-review-runs/:runId/authority",
    async (request, reply) => {
      ...
      if (
        ...
        return reply.status(403).send({
          error: "A valid active agent runtime session is required for Context review authority.",
      ...
      const parsed = contextReviewAuthorityRequestSchema.safeParse(request.body);
      if (!parsed.success) {
      ...
      try {
        return await inspectContextReviewAuthority({
          db: app.db,
          ...
          reviewedHead: parsed.data.reviewedHead,
