...
current head. Confirm the Reviewer may repair only live changed, non-protected
paths with its local GitHub identity, while only the GitHub App publishes
commit-bound reviews and only the local identity performs an exact-head squash
merge.
...
- Prepare ready, draft, fork, clean, validator-failing, semantic-failing,
  ...
  merge-failing fixtures. PR bodies are ordinary authoring context and carry no
  repair-consent block or mutation authority.
...
- Deliver draft, unsupported, App-unavailable, and malformed-but-identifiable
  bound Tree PR payloads. Confirm each fails closed without a Review Chat,
  generic audience, or generic delivery. Mark the draft ready and confirm its
  first and only Review Chat is created then. Repeat against an ordinary code
  repository plus Issue, Discussion, Commit, and GitLab events and confirm
  their generic routes are unchanged.
- Replay the delivery and send opened, reopened, ready_for_review, and
  ...
  one stable Chat. Send issue and review comments as the PR author and current
  Team human identity and confirm substantive discussion reaches the same Chat.
  Confirm App/bot self-echo and external-human comments do not wake a run, while
  unsupported title/label noise does
...
- Reassign or disable the Reviewer before a new event and before submission.
  ...
  authority, the same Chat is reused, the former Reviewer is removed as a
  speaker, and no previous Reviewer's result authorizes the replacement. Create
  a generic Chat with forged reviewer metadata and confirm it is neither reused
  nor woken.
...
- For every run, confirm the Reviewer resolves the bound repository and PR,
  ...
  snapshot, and runs Context Tree validation before semantic review. If an
  already trusted ready run becomes draft, confirm zero mutation and one
  commit-bound `COMMENT` that defers approval. A draft `opened` event still
  creates no Chat or run. Stale run, changed head, binding mismatch, and missing
  permissions fail closed without mutation or verdict; a fork remains
  read-only but may receive the appropriate commit-bound verdict.
...
- Exercise a safe repair in a same-repository branch. Confirm local git and gh
  credentials create and push only objective, decision-preserving changes in
  the intersection of live base-to-head changed files and non-protected policy.
  Confirm owners, protected structure,
  decision locks, CODEOWNERS, workflow files, and ambiguous decisions are never
  modified automatically.
- Exercise protected files, new paths, files outside the live changed set,
  forks, and ambiguous decisions. Confirm each disables mutation but still
  permits an appropriate read-only exact-head verdict. Change the live head,
  binding, assignment, or changed-path set before every
...
- For a clean current head, inspect the current required checks once and invoke first-tree tree
...
- Force local merge permission, ruleset, check, and transient provider failures.
  ...
  rolled back, no duplicate review is published, and the run does not retry,
  queue, enable auto-merge, or create a long-CI continuation.
...
- Follow the same PR from another collaboration Chat. Confirm bound Tree PR
  events still create zero generic GitHub cards while Context Reviewer dispatch
  remains one independent PR-scoped App path.
