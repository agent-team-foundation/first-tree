...
4. From a GitHub-only user's User Settings, connect Google. Verify user, organization, membership, agents, and active First Tree tokens remain unchanged.
...
8. Start a link or unlink flow, let state expire, and verify a recoverable error returns to User Settings without changing the current First Tree session.

## Evidence

- Screenshots of Login, Invite Accept, User Settings connections, disabled last-provider state, and recoverable errors.
