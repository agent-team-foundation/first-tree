...
import { FirstTreeCoreSDK, type RegisterResult } from "./sdk.js";
...
export class AgentConnection extends EventEmitter<ConnectionEvents> {
  ...
  constructor(config: AgentConnectionConfig) {
    ...
    this.sdk = new FirstTreeCoreSDK({ serverUrl: config.serverUrl, token: config.token });
