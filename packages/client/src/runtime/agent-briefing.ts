...
function sourceRepositoriesBlock(sourceRepos: ReadonlyArray<PredeclaredSourceRepo>): string {
  ...
  lines.push(
    ...
    "then retire the legacy checkout. Clear the same zero-data-loss bar for",
    "**everything** the final `rm -rf` would destroy — its linked worktrees,",
    "its own working tree, AND any local-only history in its `.git` (branches",
    "checked out in no worktree, plus stashes). Delete a tree only when it has",
    "no uncommitted changes and its HEAD is merged into `origin/<default>`;",
    "`git worktree remove` refuses the main checkout, so verify it by hand. If",
    "any check below is non-empty, stop — push / migrate that work or ask a",
    "human before deleting:",
    "",
    "```bash",
    ...
    "# the checkout ITSELF — `worktree remove` won't touch a main tree, and",
    "# `rm -rf` also drops local-only refs/stashes; clear the full bar by hand:",
    ...
    "rm -rf <legacy>                        # only after ALL of the above are clear",
    "```",
    "",
