...
function sourceRepositoriesBlock(sourceRepos: ReadonlyArray<PredeclaredSourceRepo>): string {
  ...
  lines.push(
    ...
    "then retire the legacy checkout. Clear the same safety bar for **both**",
    "its linked worktrees AND its own working tree — delete a tree only when",
    "it has no uncommitted changes AND its HEAD is already merged into",
    "`origin/<default>`, so no work is lost. `git worktree remove` refuses",
    "the main checkout, so verify that one by hand before the `rm -rf`:",
    "",
    "```bash",
    ...
    "# the checkout ITSELF — `worktree remove` won't touch a main tree, so",
    "# clear the same bar by hand before deleting it:",
    "git -C <legacy> status --porcelain     # empty ⇒ no uncommitted work",
    "git -C <legacy> merge-base --is-ancestor HEAD origin/<default>  # exit 0 ⇒ already merged",
    "rm -rf <legacy>                        # only after BOTH checks pass",
    "```",
    "",
