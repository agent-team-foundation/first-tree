...
function sourceRepositoriesBlock(sourceRepos: ReadonlyArray<PredeclaredSourceRepo>): string {
  ...
  lines.push(
    ...
    '# Prints "ok: <path>" and sets $legacy only when EVERY gate below holds.',
    "assert_legacy_target() {",
    "  name=$1 want=$2 legacy=$WS/$name",
    "  case $name in",
    ...
    "  esac",
    '  [ -e "$legacy" ] || { echo "skip: nothing at $legacy"; return 1; }',
    '  [ -L "$legacy" ] && { echo "reject: $legacy is a symlink"; return 1; }',
    ...
    '  top=$(git -C "$legacy" rev-parse --show-toplevel 2>/dev/null) \\',
    '    || { echo "reject: $legacy is not a git checkout"; return 1; }',
    '  [ "$top" = "$real" ] || { echo "reject: $legacy sits inside another repo ($top)"; return 1; }',
    '  [ "$(git -C "$legacy" rev-parse --is-bare-repository 2>/dev/null)" = false ] \\',
    '    || { echo "reject: $legacy is bare, not a flat checkout"; return 1; }',
    '  got=$(git -C "$legacy" remote get-url origin 2>/dev/null)',
    // biome-ignore lint/suspicious/noTemplateCurlyInString: shell POSIX parameter expansion (${var%suffix}) inside a recipe string, not a JS template placeholder
    '  [ "${got%.git}" = "${want%.git}" ] \\',
