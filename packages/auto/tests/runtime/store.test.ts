/**
 * Tests for `packages/auto/src/runtime/store.ts`.
 *
 * Covers:
...
import type { Inbox } from "../../src/runtime/types.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const FIXTURE = join(__dirname, "..", "fixtures", "inbox-sample.json");

function mkTmp(): string {
  return mkdtempSync(join(tmpdir(), "auto-store-"));
}
