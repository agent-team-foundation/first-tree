/**
 * auto watch TUI tests — rendered via `ink-testing-library`.
 *
 * We exercise the pure view component `AutoWatch` with canned inbox +
...
import { AutoWatch } from "../../src/commands/watch.js";
import type {
  ActivityEvent,
  Inbox,
...
describe("AutoWatch view", () => {
  it("renders header with status counts", () => {
    ...
    const { lastFrame } = render(
      <AutoWatch inbox={inbox} events={[]} />,
    );
    const frame = strip(lastFrame());
    expect(frame).toMatch(/auto/u);
  ...
  it("renders live-feed transition events with from/to labels", () => {
    ...
    const { lastFrame } = render(
      <AutoWatch inbox={inbox} events={events} />,
    );
    const frame = strip(lastFrame());
  ...
  it("renders live-feed new events", () => {
    ...
    const { lastFrame } = render(
      <AutoWatch inbox={null} events={events} />,
    );
    const frame = strip(lastFrame());
    expect(frame).toMatch(/▸ NEW/u);
