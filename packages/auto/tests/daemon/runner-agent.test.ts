...
describe("buildAgentEnv", () => {
  it("prefixes shim dir to PATH and exports BREEZE_* env vars", () => {
    ...
    expect(env.AUTO_BROKER_DIR).toBe("/opt/shim");
    expect(env.AUTO_SNAPSHOT_DIR).toBe("/snap");
    expect(env.AUTO_TASK_DIR).toBe("/task");
  });
});
...
describe("executeAgent", () => {
  ...
  it("forwards the abort signal into the spawner", async () => {
    const controller = new AbortController();
    let observedSignal: AbortSignal | undefined;
    const spawner: AgentSpawner = async ({ signal, outputPath }) => {
      observedSignal = signal;
      writeFileSync(outputPath, "BREEZE_RESULT: status=handled summary=ok");
      return { statusCode: 0 };
    };
    await executeAgent({ kind: "codex" }, fakeRequest(), {
      timeoutMs: 1_000,
      spawner,
