/**
 * Phase 3c: cross-process `gh` broker for the TypeScript auto daemon.
 *
 * Port of `broker.rs`.
 *
 * Purpose
...
broker_dir="\${AUTO_BROKER_DIR:?missing AUTO_BROKER_DIR}"
...
timeout_secs="\${AUTO_BROKER_TIMEOUT_SECS:-1800}"
...
export interface RunningBroker {
  ...
  shimDir: string;
  /** `AUTO_BROKER_DIR` passed to the agent. */
  brokerDir: string;
