...
export type AgentSpawner = (args: {
  ...
  stderrPath: string;
  /**
   * When this aborts, the spawner must terminate the child it spawned.
   * Used by the dispatcher's timeout + shutdown paths to kill agents
   * that overrun their budget instead of leaking subprocesses that can
   * keep mutating GitHub after the task is marked failed.
   */
  signal?: AbortSignal;
...
/**
 * Spawn codex or claude with the agent's env. When `signal` aborts (the
 * dispatcher's timeout / shutdown path), SIGTERM the child, then SIGKILL
 * after a 5s grace period if it's still alive — otherwise the agent can
 * keep operating on GitHub long after the task is marked failed.
 */
...
export function buildAgentEnv(
  ...
  return {
    ...
    AUTO_BROKER_DIR: request.ghBrokerDir,
    AUTO_SNAPSHOT_DIR: request.snapshotDir,
    AUTO_TASK_DIR: request.taskDir,
  };
}
