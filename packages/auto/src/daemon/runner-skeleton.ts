...
export async function runDaemon(
  ...
  try {
    config = loadAutoDaemonConfig({ cliOverrides });
  } catch (err) {
    logger.error(
  ...
  try {
    ...
    if (!identityHasRequiredScope(identity)) {
      ...
      logger.info(
        `auto daemon: identity=${identity.login}@${identity.host}`,
      );
    }
  } catch (err) {
    logger.warn(
  ...
  logger.info(
    `auto daemon: poll-interval=${config.pollIntervalSec}s host=${config.host} http-port=${config.httpPort} max-parallel=${config.maxParallel} search-limit=${config.searchLimit} allow-repo=${repoFilter.isEmpty() ? "all" : repoFilter.displayPatterns()}`,
  );
  ...
  try {
    ...
    if (agents.length > 0 && realGh) {
      ...
      logger.info(
        `auto daemon: dispatcher ready (agents=${agents.map((r) => r.kind).join(",")}, broker=${broker.shimDir})`,
      );
