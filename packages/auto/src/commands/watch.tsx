 ...
 *   - header: "auto status board" + status-count summary
 ...
 *
 * Read-only: watches `~/.first-tree/auto/inbox.json` (polled) and
 * `~/.first-tree/auto/activity.log` (tailed via fs.watch). No writes.
 *
...
function statusColor(s: AutoStatus): string {
  switch (s) {
    case "human":
      return COLOR.red;
    case "new":
...
const HeaderBanner = (): React.ReactElement => {
  // Rainbow-tinted "auto" letters; each letter a fixed colour.
...
export interface AutoWatchProps {
  inbox: Inbox | null;
  events: readonly ActivityEvent[];
}
...
export const AutoWatch = ({
  inbox,
  events,
}: AutoWatchProps): React.ReactElement => {
 ...
 * `AutoWatch` (the pure view) so ink-testing-library can render the view
...
const WatchApp = ({
  ...
  return <AutoWatch inbox={inbox} events={events} />;
};

/**
 * Entry point for `first-tree auto watch`.
 *
