...
/**
 * Shared Context-Tree kickoff plumbing. Extracted from the onboarding kickoff
 * step so the standalone build entry on the Context tab can reuse the exact same
 * "register repos → provision the binding → send the `tree` kickoff" sequence —
 * there is one build path, not a wizard-page copy of it.
 */

export type KickoffAgent = ManagedAgent;

export async function ensureKickoffRepos(organizationId: string | null, sourceRepos: readonly string[]): Promise<void> {
...
export async function startKickoffChat(args: {
  agent: KickoffAgent;
  bootstrap: string;
...
export async function startTreeSetupKickoff(args: {
  agent: KickoffAgent;
  organizationId: string;
