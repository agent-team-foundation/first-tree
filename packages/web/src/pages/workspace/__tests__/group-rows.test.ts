...
function row(overrides: Partial<MeChatRow> & { id: string; lastMessageAt: string | null }): MeChatRow {
  return {
    ...
    chatHasOpenAttentionForMe: overrides.chatHasOpenAttentionForMe ?? false,
  };
}
...
// Three-rule chat-granularity scoping predicate.
// R1 (mine-failed via failedAgentIds), R2 (explicit @<me> in unread window
// via chatHasExplicitMentionToMe + unreadMentionCount > 0), R3 (open
// Attention targeting me via chatHasOpenAttentionForMe).
// See docs/development/needs-attention-scoping.20260526.md.
describe("splitAttentionRows — three-rule predicate", () => {
  ...
  it("R2 disabled by 1-on-1 implicit auto-mention: unreadMentionCount > 0 but flag false → NOT attention", () => {
    // User's痛点 (t7): in a 1v1, agent → human plain "ack" bumps the v1
    // red-dot counter but `metadata.mentions` is empty, so the server
  ...
  it("R3: open attention targets me → attention bucket, needs_you tier", () => {
    const rows = [
      row({
        id: "r3",
        lastMessageAt: null,
        chatHasOpenAttentionForMe: true,
      }),
    ];
    const { attention } = splitAttentionRows(rows);
    expect(attention.map((r) => r.chatId)).toEqual(["r3"]);
    expect(attention[0] && rowAttentionReason(attention[0])).toBe("needs_you");
    // The needs-you badge faithfully reflects R3 in the new model (no more
    // "bucket position alone signals R3" carve-out).
  ...
  it("priority: failed > needs_you > mention", () => {
    const rows = [
      row({ id: "m", lastMessageAt: null, unreadMentionCount: 1, chatHasExplicitMentionToMe: true }),
      row({ id: "n", lastMessageAt: null, chatHasOpenAttentionForMe: true }),
  ...
  it("priority: needs_you beats mention on the same row (R2 + R3)", () => {
    const rows = [
      row({
        id: "nm",
        lastMessageAt: null,
...
// check the new bools with strict `=== true` so an `undefined` value from
...
describe("splitAttentionRows — version skew (old server, new web)", () => {
  it("missing chatHasExplicitMentionToMe (undefined) → R2 disabled", () => {
    // Simulate old server: the new field is missing. Cast through `unknown`
    // because TS won't let us omit a required field.
    const stale = {
  ...
  it("missing chatHasOpenAttentionForMe (undefined) → R3 disabled", () => {
    const stale = {
      ...row({ id: "stale-r3", lastMessageAt: null }),
    };
    delete (stale as { chatHasOpenAttentionForMe?: boolean }).chatHasOpenAttentionForMe;
    const { attention, rest } = splitAttentionRows([stale]);
    expect(attention).toEqual([]);
    expect(rest.map((r) => r.chatId)).toEqual(["stale-r3"]);
  });

  it("missing fields still let R1 (failedAgentIds) fire", () => {
    const stale = {
