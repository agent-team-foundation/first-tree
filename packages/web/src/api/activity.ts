 ...
 * which runs the provider's official login and surfaces progress (device code
 * / success) by re-PATCHing capabilities — so the caller just polls
 * {@link listClients} / {@link getClientCapabilities} afterwards and reads
 * `entry.pendingDeviceAuth` then the flipped `state`.
 */
