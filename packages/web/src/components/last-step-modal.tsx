...
export function LastStepModal({ agent, open, onClose, onBound }: Props) {
  ...
  // Append --agent-id/--agent-name so `client connect` pins the new agent
  // on this computer (otherwise the runtime starts with no agent.yaml, no WS
  // opens, no bind happens, and this modal hangs on "Waiting…"). The server
  // command is just the bootstrap half; the bridge to a specific agent is
  // owned by this flow.
  const baseCommand = tokenQuery.data?.command ?? "";
  const command =
    baseCommand && agent.name
      ? `${baseCommand} --agent-id ${agent.uuid} --agent-name ${shellQuote(agent.name)}`
      : baseCommand;
