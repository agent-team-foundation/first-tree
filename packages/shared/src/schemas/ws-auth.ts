 ...
 *
 * Required by clients in the 0.10.4 ~ 0.14.x range: those builds read
 * `wsInboxDeliver` here to decide whether to skip the local HTTP poll loop
 * and rely on `inbox:deliver` push frames. The 0.15.0+ runtime ignores the
...
export const serverCapabilitiesSchema = z
  .object({
     ...
     * build — the legacy `new_message` doorbell path was removed in 0.15.0,
     * so there is no negotiation: it's signalled to the client purely so
     * 0.10.4 ~ 0.14.x clients suppress their local 5s HTTP poll.
     */
