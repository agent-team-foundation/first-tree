...
export const serverConfigSchema = defineConfig({
  ...
  contextTree: {
    repo: field(z.string(), {
      env: "FIRST_TREE_CONTEXT_TREE_REPO",
  ...
  github: {
    ...
    webhookSecret: field(z.string().optional(), {
      env: "FIRST_TREE_GITHUB_WEBHOOK_SECRET",
      secret: true,
    }),
  },
  cors: optional({
  ...
  rateLimit: optional({
    ...
    webhookMax: field(z.number().default(60), { env: "FIRST_TREE_RATE_LIMIT_WEBHOOK_MAX" }),
  }),
});
