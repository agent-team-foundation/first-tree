...
export function checkServerConfig(): CheckResult {
  ...
  const hasEnv = !!(
    process.env.FIRST_TREE_DATABASE_URL ||
    process.env.FIRST_TREE_CONTEXT_TREE_REPO ||
    process.env.FIRST_TREE_GITHUB_TOKEN
  );
  ...
  return { label: "Config", ok: false, detail: "no config file or env vars — run: first-tree-core config setup -s" };
}
...
export async function checkDatabase(): Promise<CheckResult> {
  ...
  if (typeof dbUrl !== "string" || !dbUrl) {
    return { label: "Database", ok: false, detail: "not configured (FIRST_TREE_DATABASE_URL or config file)" };
  }
...
export async function checkGitHubToken(): Promise<CheckResult> {
  ...
  if (typeof token !== "string" || !token) {
    return { label: "GitHub Token", ok: false, detail: "not configured (FIRST_TREE_GITHUB_TOKEN or config file)" };
  }
...
export async function checkContextTreeRepo(): Promise<CheckResult> {
  ...
  if (typeof repo !== "string" || !repo) {
    return { label: "Context Tree", ok: false, detail: "not configured (FIRST_TREE_CONTEXT_TREE_REPO or config file)" };
  }
  if (typeof token !== "string" || !token) {
...
export function checkClientConfig(): CheckResult {
  ...
  if (hasEnv) return { label: "Config", ok: true, detail: "via environment variables" };
  return { label: "Config", ok: false, detail: "no config file or env vars — run: first-tree-core config setup -c" };
}
...
export function checkAgentConfigs(): CheckResult {
  ...
  if (!existsSync(agentsDir)) {
    return {
      label: "Agents",
      ok: false,
      detail: "no agents configured — run: first-tree-core client add <name> --token <token>",
    };
  }
  try {
