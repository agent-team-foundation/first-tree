...
import { buildApp } from "@agent-hub/server";
import type { Config } from "@agent-hub/server/config";
      ...
      if (!isDockerAvailable()) {
        throw new Error(
          "Docker is not available.\n\n" +
            "  Agent Hub needs PostgreSQL. Two options:\n\n" +
            "  1. Install Docker → https://docs.docker.com/get-docker/\n" +
            "     Then re-run: agent-hub server start\n\n" +
...
function resolveWebDist(): string | undefined {
  ...
  try {
    ...
    status("Web", "building...");
    execSync("pnpm --filter @agent-hub/web build", {
