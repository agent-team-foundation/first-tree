...
| 我想要... | 方式 | 指南 |
|-----------|------|------|
| 本地试用 | `first-tree-core server start` | 上方快速开始 |
...
| 在其他机器运行 Agent | `first-tree-core client start` | [部署指南](docs/deployment-guide.md#client-setup) |
| 使用托管 PostgreSQL | Supabase | [部署指南](docs/deployment-guide.md#managed-postgresql-supabase) |

## 诊断

```bash
first-tree-core server doctor   # 检查服务器环境就绪状态
first-tree-core client doctor   # 检查客户端环境就绪状态
first-tree-core status          # 服务器健康状态 + 已配置的 Agent
```
...
pnpm --filter @first-tree-core/server dev   # 启动服务器（开发模式）
pnpm --filter @first-tree-core/web dev      # 启动管理后台（开发模式）
