...
- **一键启动** — `first-tree-hub start` 自动拉起 PostgreSQL、跑 migration、静默创建本机管理员（不显示密码），浏览器自动打开 Workspace
...
一条命令完成 PostgreSQL 拉起、migration、自动创建本机管理员（不显示密码）、嵌入式 Client 启动、浏览器自动打开 `http://127.0.0.1:8000`。按 Ctrl+C 停。

## 部署

| 我想要... | 方式 | 指南 |
|-----------|------|------|
| 本地试用 | `first-tree-hub start` | 上方快速开始 |
