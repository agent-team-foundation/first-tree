...
Ownership rules (from .context-tree/ownership-and-naming.md):
...
def collect_entries(root: Path) -> list[tuple[str, list[str]]]:
    ...
    for dirpath in sorted(root.rglob("*")):
        if not dirpath.is_dir():
            continue
        if any(part in SKIP or part.startswith(".") for part in dirpath.relative_to(root).parts):
            continue
...
def generate(check: bool = False) -> int:
    ...
    if check:
        if CODEOWNERS_PATH.exists() and CODEOWNERS_PATH.read_text() == content:
            ...
            print("CODEOWNERS is out-of-date. Run: python .context-tree/generate_codeowners.py")
            return 1
