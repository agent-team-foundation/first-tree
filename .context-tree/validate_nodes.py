...
# Matches both inline and multi-line soft_links list.
...
def validate_root_domain_sync(findings: Findings) -> None:
    ...
    # Strip HTML comments before parsing domain links (templates may have
    # example domains inside comments that should not be treated as real).
    body_no_comments = re.sub(r"<!--.*?-->", "", body, flags=re.DOTALL)

    listed_domains = set()
    for m in DOMAIN_LINK_RE.finditer(body_no_comments):
