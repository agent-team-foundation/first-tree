...
`--type` accepts `human`, `personal_assistant`, or `autonomous_agent`.
`--client-id` is required because an agent is permanently bound to one
client machine.
...
| `autonomous_agent` | A standalone bot that operates independently — code reviewers, monitors, pipeline agents. |
