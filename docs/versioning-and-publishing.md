...
One package is published to npm; the rest are `private: true` and
bundled into the published artifact at build time via `tsdown`. The
...
| `@agent-team-foundation/first-tree-hub-shared` | `packages/shared` | No (`private: true`) | Do not bump — version is inert. `tsdown` inlines `shared` into the `command` tarball, so changes here are released by bumping `command` (when `command` / `client` import them). |
...
When unsure, grep `packages/command/src` and `packages/client/src` for the export name. No hits → server/web-only → no `command` bump.
