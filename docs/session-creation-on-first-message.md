...
### R7. 即时性范围限于 Hub UI 本机发送路径

M 方案的 Step 1b 调用 upsertSessionState 时传 `notifier: undefined`,即 **server 这次预测写入不发 PG NOTIFY**。这意味着不同入口的列表即时刷新行为有差:

| 入口 | 列表即时刷新 | 兜底机制 |
|---|---|---|
| **Hub UI 本机 sendMessage** | ✅ 即时 | sendMut.onSuccess invalidate sessionsKey |
| Adapter (Feishu/Slack) 进来的消息 | ❌ 不即时 | client 处理后上报 + 10s 轮询(5-10s 延迟) |
| Agent ws 进来的消息 | ❌ 不即时 | 同上 |
| GitHub Webhook 等 | ❌ 不即时 | 同上 |

**为什么这样**:

- 用户的核心 bug 是 Hub UI 自己的 new chat 路径
- 其他入口的列表延迟**是当前已存在的行为**,M 方案不引入 regression
- 让 server 预测写入也发 NOTIFY 需要把 `notifier` 注入 sendMessage,改动面变大(影响所有 caller),不在本次 scope

**未来扩展方向**:如要让所有入口都即时刷新,可在后续 PR 中:

1. 把 `notifier` 加到 sendMessage 的可选参数
2. Step 1b 调 upsertSessionState 时传入真实 notifier
3. Web admin ws 监听 `session:state` NOTIFY 即可自动刷新
