...
- Server `--detach` flag —— service 模式(下面)已覆盖 daily user 场景,单独的 detach flag 价值小。

#### Service 模式(opt-in,讨论后增加)

原始 `start` 命令是纯前台的。为了照顾 daily user(不只是 evaluator)而又不强制他们用 tmux/nohup,我们加一个**opt-in 的 service 模式**,跟 Multica 的 daemon 对称:

```
first-tree-hub start --service       # 一次性装好,立即退出
                                      # daemon 在后台跑(launchd / systemd-user)

first-tree-hub service status        # 它在跑吗?
first-tree-hub service logs -f       # 跟随 rotating 日志
first-tree-hub service stop          # 停服务但不卸
first-tree-hub service uninstall     # 删 plist/unit + 停服务

first-tree-hub login                 # 按需打印一份新的 magic URL
                                      # 不论 `start` 是 inline 还是 service,都能用
```

Service 模式下,bootstrap URL 仍在每次 boot 时签发,但写到 rotating log 而不是用户的 terminal。用户通过 URL 登录一次后,**浏览器 cookie 自然持久**(refresh-token TTL ≈ 几周),日常只用 `localhost:8000` —— 只有 cookie 过期 / 换浏览器 / 偶尔需要恢复时,才跑 `first-tree-hub login` 拿新 URL。

这套保留了 Q1 "完全不暴露 username/password" 的承诺,同时去掉"重启电脑就要重开 terminal"的摩擦。
...
- **C8.** 给本地 `start` 加 opt-in service 模式(讨论后新增,部分覆盖原 Q2-A)。
  - `first-tree-hub start --service` —— 装 launchd plist(macOS)/ systemd-user unit(Linux),跑 service 后立即退出。重启自启。
  ...
  - Service 内部调的是同一份 `start` orchestration —— Docker preflight、auto-admin(首次)、每次 boot 签 bootstrap token、嵌入 ClientRuntime。差异:stdout/stderr 进 log 文件,bootstrap URL 写到一行已知格式的 log 里给 `login`(C9)再签发用。
  - Postgres 容器生命周期跟 service 解耦——`service stop / uninstall` 不动 Docker 容器。
- **C9.** 加 `first-tree-hub login` 命令(讨论后新增,推翻 Q1 原决议中"不加恢复命令"的部分)。
  - 连本地 server,给 admin 签一份新的 single-use bootstrap token,打印 magic URL。
  - 不论 `start` 是前台还是 service 模式都可用。
  - 替代原"Ctrl+C + 重启"恢复路径;UX 上等价于 Multica 的"邮件再发一次验证码",但不需要离开 stdout。
  - 约 20 行代码:复用现有 `core/admin.ts` + 跟 C2 同一条 token 签发路径。

(原 C6 client-retry-backoff 和 C7 localhost-no-service 删除——都被 C2 的嵌入式 client 模型自动消解。)

## 5. 分阶段顺序

- **Phase 1(本地版,已完整 spec):** C2(`first-tree-hub start` 前台 + bootstrap endpoint + Web URL handler)+ C8(opt-in service 模式 + service CLI 命令组)+ C9(`login` 命令)+ D1 的本地段 + README Quick Start 改写。独立可发,跟任何 hosted 决议解耦。
...
| ID | 问题 | 决定 | 理由 |
|---|---|---|---|
| Q1 | auto-admin + bootstrap UX | username/password/org 全程不向用户展示,不落盘。每次 `start` 签新 URL。~~不加恢复命令~~ | (见下方 Q1-revisit) |
| Q2 | server 是否做成 service | ~~否(Q2-A),前台进程~~ | (见下方 Q2-revisit) |
...
| **Q2-revisit** | server 是否做成 service(对比 Multica 后重审) | **opt-in service 模式。** 默认仍前台(`first-tree-hub start`)。新增 `--service` flag + `first-tree-hub service {install,uninstall,status,logs}` 命令组。Service 模式重启自启(launchd / systemd-user)。 | 原 Q2-A 把本地用户窄化成 evaluator,但 daily user(用本地 Hub 跑个人项目)需要持久化;"每次重启都要开 terminal" 是真摩擦。Opt-in 让 evaluator 路径不变 |
| **Q1-revisit** | cookie 过期 / URL 丢失的恢复(在 service 模式下重审) | **加 `first-tree-hub login` 命令。** 按需打印新 magic URL。Service 模式下 bootstrap URL 进的是 log 文件,`login` 是用户拿 URL 的入口,不必重启 service。 | 原 Q1 的"不加恢复命令"假设了前台模式;service 模式下 magic URL 在 terminal 不可见,这条假设不成立。路 1(cookie 持久化 + 偶尔 `login`)保留了"完全不暴露密码"的承诺 |
