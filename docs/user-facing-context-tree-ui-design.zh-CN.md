...
> 在 Hub 中提供一个面向用户的 Context Updates surface,让用户优先看到 Context Tree 最近如何生长、哪些变化可能被 agents 用来判断和行动,再通过 Context Tree Overview 理解变化在团队认知树中的集中位置。
...
4. **Overview as proof**
   Context Tree Overview 保留,但职责是全局态势和结构证明,不是当前 update detail,也不是主解释器。
...
| Structure visible | 这些变化集中在树的哪里? | Context Tree Overview 作为全局态势 |
...
│ Shows where recent context updates are concentrated across the tree     │
│ root                                                                   │
│ ├─ agent-hub       4                                                    │
│ │  ├─ web-console   ~                                                   │
│ │  ├─ breeze        +                                                   │
...
- **Context Tree Overview**:第二屏全宽辅助区域,展示变化在树上的集中位置和整体结构。
...
Context Tree Overview 是全局态势视图,不是当前 update detail,也不是主体验。

规则:

- 默认展示整棵树或主要 domain。
- changed domain 显示聚合计数。
- 选中 Update Card 时,Overview 高亮对应节点和 ancestor path。
- 点击有直接 update 的节点时,选中对应 Update Card;点击聚合 domain 时,只高亮该 domain,不改变当前 Selected Change。
...
2. **Web Updates-first baseline**
   Header、Change Summary、Context Updates、Selected Change、source preview。此阶段即使没有 Context Tree Overview,也能传达核心价值。
