...
| Q7 | URL delivery + auth model | Single-step: open `http://127.0.0.1:8000` in the browser (via `open` / `xdg-open` / `start`, skipped on `--no-open` / SSH / non-TTY) + always print the URL to stdout. Web auth guard handles authentication via Web `/login` → `POST /api/v1/auth/local-bootstrap` (loopback-only, gated on `req.ip` + no `X-Forwarded-*`). **Trust boundary: loopback access = local admin.** | Earlier draft layered auto-open + stdout + clipboard around a magic URL with a bootstrap token. The loopback-trust model collapses all of that: the URL is just `http://127.0.0.1:8000`, no token to copy, no clipboard fallback needed. Endpoint is disabled in hosted-mode deployments |
