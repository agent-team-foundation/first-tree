...
- **C10. Out-of-band CLI auth via `local-bootstrap` (B3 / Q10).** Add a shared helper `core/auth.ts:obtainCliJWT(serverUrl)` used by every command that needs an authenticated server call (`agent send`, `agent chats`, `agent status`, etc.). Logic:
  1. Read `credentials.json`. If access token's `exp` is still in the future → use as-is.
  2. Else (access expired or no creds.json), if `serverUrl` is loopback (`127.0.0.1` or `localhost`) → POST `/api/v1/auth/local-bootstrap`, return the new pair **in-memory only — do NOT write to `credentials.json`**.
  3. Else (hosted-mode CLI) → POST `/api/v1/auth/refresh` with the cached refresh token (existing behaviour; pre-existing race with the client daemon if installed, out of B3 scope).

  **Invariant:** in local mode, the daemon is the **sole writer** of `credentials.json`. Out-of-band CLI is read-only on disk; mints fresh JWTs in memory when the cached access token has expired. This avoids the refresh-token-rotation race between daemon and CLI.

  **DB pollution:** only the cold-mint branch (step 2) creates a new refresh-token row. Bound by access-token TTL × user activity — at TTL = 1h, a daily user produces ~10 rows/day. Not a Phase 1 concern; if it becomes a friction point, add a per-CLI-process in-memory cache with sub-second TTL.
...
| Q10 | Out-of-band CLI auth (B3) | `core/auth.ts:obtainCliJWT()` reads `credentials.json` read-only; if access valid → use; else, if local-mode → `local-bootstrap` (in-memory only, no disk write); else → standard refresh (hosted, pre-existing). **Daemon is sole writer of `credentials.json`** in local mode. | Daemon and CLI sharing `credentials.json` would race on refresh-token rotation (single-use refresh tokens — whoever refreshes first invalidates the other's). Loopback access lets CLI mint fresh in-memory pairs without touching shared state. Hosted-mode race is pre-existing, deferred |
