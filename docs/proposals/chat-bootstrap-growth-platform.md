...
- 明确去掉 bootstrap 层的全局 `kind` 设计，不再用 `intro`、`work`、`tree` 这类预设枚举承载业务意图。
...
- 去掉新 bootstrap core 和新 domain wrapper 对全局 `kind` 的依赖。
...
当前 `POST /me/onboarding/kickoff` 的 shared schema 使用 `kickoffKindSchema = intro | work | tree`。Server service 再用这个 `kind` 做两件事：

- 生成 topic，例如 intro 对应 “Meet your agent”，tree 对应 “Set up team context”。
- 拼接幂等 key，例如 `<humanAgentId>:<targetAgentId>:<kind>`，quickstart 再追加 campaign。

这个设计解决过 onboarding 内部区分不同启动场景的问题，但它不适合作为长期平台抽象。原因是：

- `kind` 是全局预设，新增业务很容易继续往枚举里塞 `campaign_scan`、`github_review`、`security_scan`。
- `kind` 同时影响 topic、幂等 key 和业务意图，职责混在一起。
- quickstart 真正需要区分的是 campaign + repo/run，不是 `work` 这个通用类别。
- Context Tree setup 真正需要的是自己的 setup 状态和 key，不应该借 onboarding 的 `tree` kind 表达。

因此，第一阶段应该明确把全局 `kind` 从新架构里去掉：

- `chatBootstrapService` 输入不包含 `kind`。
- 新 onboarding wrapper 直接传入 onboarding 自己的 `bootstrapKey`、`topic`、`bootstrapContent`。
- 新 quickstart campaign wrapper 直接传入 campaign/repo 语义的 `bootstrapKey`、`topic`、`bootstrapContent`。
- 旧 `/me/onboarding/kickoff` 可以短期兼容 `kind`，但只能作为 legacy wrapper 内部映射，不再扩展新的 enum value。

换句话说，`kind` 不应该是未来的业务分类系统；业务意图由 domain service、skill 和 bootstrap 内容表达。
...
- 本 proposal 不改变现有 chat model。
- 去掉 bootstrap 层全局 `kind`，不再把业务意图收敛到一个共享枚举里。
...
注意这里刻意没有 `kind`。`topic` 和 `bootstrapKey` 由调用方显式传入，避免 core 根据全局预设推导业务语义。
...
这也意味着 `kind` 不再参与 core key 生成。旧 route 为了兼容可以继续把旧 `kind` 映射进 legacy key，但 quickstart 和后续 campaign 不应该再依赖 `work` 这类全局类别。
...
- `chatBootstrapService` input 不包含 `kind`；topic、bootstrapKey、bootstrapContent 都由 domain wrapper 显式传入。
...
- Quickstart 新路径不再传 `kind: "work"`；campaign intent 由 campaign wrapper、skill 和 bootstrap 内容表达。
...
- 不把 `intro | work | tree` 扩展成新的全局业务分类系统。
...
1. 是否同意第一阶段只做最小必要边界修正：内部抽 helper、去掉新 bootstrap core 的全局 `kind`、quickstart 改走自己的 wrapper、修正 repo 级幂等 key？
2. 是否同意不新增 public generic chat bootstrap API，避免把内部实现提前产品化？
3. 是否同意旧 `/me/onboarding/kickoff` 可以短期兼容 `kind`，但不再扩展新的 enum value，且新 quickstart/campaign 路径不再依赖它？
...
- 新 bootstrap core 不再有全局 `kind` 输入。
