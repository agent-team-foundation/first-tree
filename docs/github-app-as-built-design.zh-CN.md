# GitHub App 迁移：As-Built 设计（PR-322 + PR-323 + 延后的 webhook PR）

**最初是为单体 PR #300 撰写的。** PR #300 之后已被**关闭并拆分为三个更小的 PR**（决策：2026-05-12）。其中 auth + UI 部分以 **PR-322 + PR-323** 发布；webhook 部分**延后到一个由不同负责人接手的独立 PR**。

| PR | 范围 | 状态 |
|----|-------|--------|
| [#322](https://github.com/agent-team-foundation/first-tree-hub/pull/322) | 基础设施 —— schema（`github_app_installations`）、service 原语（`services/github-app.ts`、`services/github-app-installations.ts`、`services/oauth-state.ts`）、boot guard、env 配置 | **✅ 已合并** |
| [#323](https://github.com/agent-team-foundation/first-tree-hub/pull/323) | OAuth 流程重写 + Settings UI + admin API + auth 侧 D3 切除（删除遗留 OAuth client） | **⏳ Open**（评审进行中） |
| Webhook PR（待定） | `/api/v1/webhooks/github` App 端点 + 分发 + webhook 侧 D3 切除（删除遗留 `/webhooks/github/:orgId` + 移除 `github_integration` namespace） + 与 #304 对齐的 mention-only 路由 | **⏸ 已延后** —— 由不同负责人接手 |

**读者对象：** 评审 auth + UI 工作以决定是否批准上生产的资深架构师 / 工程负责人，以及最终 webhook PR 的评审者 —— 后者将以本文档作为设计上下文。

**如何阅读本文档：**
- **没有**显式 "deferred" 标记的章节描述的是落在 PR-322 + PR-323 的 auth + UI 工作。那部分代码就是真相之源 —— `file:line` 引用可在合并后的 main（PR-322 之后）或 `feat/github-app/02-oauth-and-ui`（PR-323）上解析。
- 标记为 **🔮 deferred to webhook PR** 的章节描述的是尚未启动的 webhook 工作的设计意图。这些章节里的代码引用只有在 webhook PR 落地后才能解析；在那之前它们只是设想，取自最初的 PR-300 rollup 分支（`ship/pr-300-rollup`）—— 设计最早就是在那里做原型的。

文中提到的 "design doc §X" 指 `docs/github-app-design-zh.md`（PR #295 上的上游提案）—— 作为历史背景保留。
...
**Delete 处理（`services/github-app-installations.ts:300-302`）：**

早先的 60 秒宽限窗口（意在吸收延迟的 `delete` 事件）在一次 codex challenge 暴露出一个更糟的
失败后被回退了：一次真实的 install + 立即 uninstall（60 秒内）会永久留存 —— handler 返回 200
（所以 GitHub 从不重投），Hub 侧的行从未被清理。当前实现是无宽限的硬 DELETE。

宽限窗口本想解决的那个原始 race 其实并不存在：GitHub 每次 install 都铸造一个新的
`installation.id`，所以针对 id N 的延迟 `deleted` 不会抹掉一个新的 re-install（它携带 id M ≠ N）。
同 id 的重放由 `processed_events` dedup 处理。

一个残留的洞 —— `deleted` 之后到达的过期 `created` 会让行复活 —— 在 jsdoc 里有承认，并作为
follow-up #314 跟踪。

### 4.c Installation 绑定模型（1:1 hub_org ↔ install）

**绑定不变式**（D2）：每个 `installation_id` 至多绑到一个 `hub_organization_id`，每个
`hub_organization_id` 至多持有一个 installation。
...
3. **race-safe 的 `bindInstallationToOrg`** —— 通过行锁串行化的条件 UPDATE。

**race-safe bind（`services/github-app-installations.ts:145-213`）：**

最初的实现是 SELECT-then-UPDATE：读这一行，校验 "仍然为 null 或已经是我的"，再 UPDATE。同一个
未绑定 installation 但不同 Hub org 的两个并发 callback 会都看到 `hub_organization_id IS NULL`，
都通过校验，然后第二个 UPDATE 会悄无声息地覆盖。Codex P0-3 标出了这一点；H2 加上了反向情形。
...
**实现：** `packages/server/src/api/webhooks/github-app.ts`。

**HMAC 校验** 复用 `webhooks/github.ts:19-31` 里的 `verifyGithubWebhookSignature` —— 与遗留的
逐 org 端点同一算法，只是 secret 来源不同。比较使用 `timingSafeEqual` 在等长 buffer 上进行；
失败路径抛 `UnauthorizedError`。

**Body 解析：** 限定作用域的 `application/json` content-type parser（`buffer` 模式），这样原始
body 被保留以供 HMAC 使用。与遗留端点同一模式。
...
**Dedup 表：** `processed_events(event_id, platform)`。`claimEvent` helper 使用
`INSERT ... ON CONFLICT DO NOTHING RETURNING event_id` —— 当且仅当在同一个 `x-github-delivery`
GUID + platform `"github-app"` 下没有先前的 delivery 被处理过时，claim 成功。这是已有的基础设施；
本 PR 原样复用。
