...
1. Human says: "Let's add SSO to Kael."
2. Agent reads relevant tree nodes (e.g., `kael/platform/`, `kael/environment/`).
...
- "Add SSO support" → `kael/platform/` (auth decision), soft links to `kael/environment/` (extension/desktop auth flows)
- "Support PPTX parsing" → `kael/knowledge/` (ingestion). Clear, single domain.
- "Q3 developer campaign" → `marketing/` (go-to-market), soft link to `kael/` (feature positioning)
- "Agent remembers user preferences" → `kael/agent/` (memory)
- "Hire a frontend engineer" → `people/hiring/` (role decision), soft link to `kael/chat/` (the team they'd join)
