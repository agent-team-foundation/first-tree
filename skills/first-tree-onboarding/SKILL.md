...
| `tree init` exits 0 but `tree status` still reports no workspace                   | `init` writes `workspace.json` only when scope is workspace-level AND the tree is an immediate child of the source root. For lone single-repo cases, manually create `<workspaceRoot>/.first-tree/workspace.json` from the spec template (see `references/cli-quickref.md`). |
...
| Phase C: tree was published to remote but local sibling dir is missing             | Confirm `<workspaceRoot>/<manifest.tree>` is in `workspace.json.sources` resolution. If `boundSources[?].present === false` for the tree's name, `git clone` it as a sibling and re-run Phase C.                                                        |
