...
| `tree init` exits 0 but `tree status` still reports no workspace                   | `init` writes `workspace.json` only when scope is workspace-level AND the tree is an immediate child of the source root. For lone single-repo cases this is expected — go back to Phase B's lone-repo flow: `cd ..` to the parent dir, run `first-tree tree migrate-to-w1 --dry-run` to preview, then `first-tree tree migrate-to-w1` to write the manifest and strip the legacy `source.json` / `bindings/` / framework block. Do NOT hand-create `workspace.json` here — that bypasses the cleanup and leaves the workspace in a half-migrated state. (The hand-create path in `references/cli-quickref.md` is for recovering a corrupted manifest in an existing W1 workspace only, not for fresh onboarding.) |
