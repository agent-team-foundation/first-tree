...
1. Decide tree mode. Defaults below — only ask the user if marked ⚠.
   - **Lone source repo (no parent workspace dir yet)** → W1 requires source + tree as siblings under a workspace root, so the source has to live inside that root before `init` can write the manifest. Pre-create the workspace dir, move the source into it, then init from there:
     ...
     3. `cd <workspace-name>`.
     4. `first-tree tree init --tree-path ./<tree-name> --workspace-id <slug>` (add `--tree-url <url>` to bind an existing remote tree instead of scaffolding a new one).
   - **Workspace root with multiple child repos (cwd is already the workspace dir)** → `first-tree tree init --workspace-id <slug-of-source-root>`. Adds `--tree-url <url>` for an existing remote tree.
...
| `tree init` rejects `--scope repo`                                                 | `--scope repo` is removed under W1. Follow Phase B's lone-source recipe: pre-create the workspace dir, `mv` the source in, `cd` in, then run `first-tree tree init --tree-path ./<tree> --workspace-id <slug>`. |
| `tree init` exits 0 but `tree status` still reports no workspace                   | `init` writes `workspace.json` only when the tree resolves to an immediate child of cwd. Common cause: init ran from inside the lone source repo (which produces a sibling tree and no manifest). `migrate-to-w1` does not detect this layout — it looks for `.first-tree-workspace`, `bindings/`, or `source.json`, none of which the current init writes. Recover by undoing the init and following Phase B's lone-repo flow: show the user what init wrote (the new sibling `<source>-tree/` dir; `AGENTS.md` / `CLAUDE.md` / `WHITEPAPER.md` / `.agents/` / `.claude/` inside the source) and ask before deleting. After cleanup, pre-create the workspace dir, `mv` the source in, `cd` in, and run `first-tree tree init --tree-path ./<tree> --workspace-id <slug>`. Do NOT hand-create `workspace.json` here — the hand-repair path in `references/cli-quickref.md` is for recovering a corrupted manifest in an existing W1 workspace only, not for fresh onboarding. |
...
| Phase C: source repo has no README and < 5 commits                                 | Generate only the minimal root routing skeleton from observed source dirs, mark every other field `# unverified — source repo too sparse to infer`, and keep supporting groups such as `members/` separate from domains. Open the PR anyway so the user can fill in. |
